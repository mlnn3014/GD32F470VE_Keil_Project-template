...
static void rs485_parse_char(char ch)
{
    ...
    if ((received_len >= MSG_ASCII_MIN) &&
        packet_end_is_ok(rx_msg_buf, received_len) &&
        ((total_len == 0) || (received_len >= total_len)))
    {
    ...
    // 不能因为长度字段到了就提前丢帧。K-02 会故意发长度错误帧。
}
...
void rs485_task(void)
{
    ...
    if ((count == 0) &&
        (received_len >= MSG_ASCII_MIN) &&
        (total_len > received_len) &&
        packet_end_is_ok(rx_msg_buf, received_len) &&
        ((uint32_t)(systick_get_ms() - last_rx_ms) >= MSG_TAIL_WAIT_MS))
    {
        rx_msg_buf[received_len] = '\0';
        rs485_command_parse(rx_msg_buf);
