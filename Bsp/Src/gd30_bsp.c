...
/* PT100 sample board header: PE2=SCK, PE4=CS, PE5=MISO(DOUT/DRDY), PE6=MOSI(DIN). */
...
#define GD30_CS_DELAY_COUNT   64U

static uint16_t gd30_extref_register;

static void gd30_cs_delay(void)
{
    volatile uint32_t count = GD30_CS_DELAY_COUNT;
...
static int gd30_transfer16_word(uint16_t value, uint16_t *received)
{
    ...
    *received = spi_i2s_data_receive(GD30_SPI);
    return 0;
}
