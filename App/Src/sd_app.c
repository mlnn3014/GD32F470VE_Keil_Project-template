...
int sd_mkdir(const char *path)
{
    ...
    if ((FR_OK == result) && ((info.fattrib & AM_DIR) != 0))
    {
        return 0;
    }
...
int sd_mkdirs(const char *path)
{
    ...
    if ((len == 0) || (len >= SD_PATH_BUFFER_SIZE))
    {
        return -1;
    }

    (void)memcpy(temp, path, len + 1);
...
int sd_append_file(const char *path, const void *data, uint32_t len)
{
    ...
    result = sd_file_open(&file, path, 1);
    if (result == 0)
    {
...
void sd_print_info(void)
{
    ...
    block_count = (card_info.card_csd.c_size + 1) * 1024;
...
static int sd_self_test_file(const char *path, const char *message)
{
    ...
    result = sd_read_file(path, readback, sizeof(readback) - 1, &read_len);
    if (result != 0)
    {
        return result;
    }
